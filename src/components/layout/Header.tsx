  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-20 py-4">
          ...
          <Link href="/" className="group flex items-center">
            <div className="relative w-auto h-10">
              <Image
                ...
                alt="Casa Flora Branding e Design"
                width={160}
                height={40}
          ...
          >
            <div className="w-5 h-5 relative">
              <span className={`absolute block w-full h-0.5 bg-current transform transition-all duration-200 ${isMenuOpen ? 'rotate-45 top-2.5' : 'top-1'
                }`} />
              <span className={`absolute block w-full h-0.5 bg-current transform transition-all duration-200 top-2.5 ${isMenuOpen ? 'opacity-0' : 'opacity-100'
                }`} />
              <span className={`absolute block w-full h-0.5 bg-current transform transition-all duration-200 ${isMenuOpen ? '-rotate-45 top-2.5' : 'top-4'
                }`} />
            </div>
          </button>
        </div>
        ...
        {/* Mobile Menu Clean */}
        <div className={`lg:hidden absolute top-full left-0 right-0 bg-off-white/98 backdrop-blur-xl border-b border-light-gray/20 transform transition-all duration-300 origin-top ${isMenuOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'
          }`}>
          <nav className="py-6">
